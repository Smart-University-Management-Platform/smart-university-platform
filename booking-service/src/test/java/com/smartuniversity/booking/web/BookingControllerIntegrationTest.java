...
class BookingControllerIntegrationTest {
    ...
    void createReservationShouldPreventOverbooking() throws Exception {
        // Use future dates (1 day from now) to pass validation
        Instant start = Instant.now().plusSeconds(86400); // 1 day from now
        Instant end = start.plusSeconds(3600); // 1 hour duration
    ...
    void concurrentReservationAttemptsShouldOnlyAllowOneSuccess() throws Exception {
        // Use future dates (2 days from now) to pass validation
        Instant start = Instant.now().plusSeconds(172800); // 2 days from now
        Instant end = start.plusSeconds(3600); // 1 hour duration
